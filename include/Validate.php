...
class NamedValidator extends Validate {
	...
	private function removeName() {
		return $this->value
			->ifSuccess(function($v) {
				return new Success(
					$v
					->ifSuccess(function($inner) {
						return new Success(isset($inner[$this->name]) ? $inner[$this->name] : null);
					})
					->ifError(function($inner) {
						return new Failure(isset($inner[$this->name]) ? $inner[$this->name] : null);
					})
				);
			})
			->ifError(function($inner) {
	...
	private static function readdName($val, $name) {
		return $val
		->ifSuccess(function($v) use ($name) {
			return new Success(
				$v
				->ifSuccess(function($inner) use ($name) {
					return new Success([$name => $inner]);
				})
				->ifError(function($inner) use ($name) {
					return new Failure([$name => $inner]);
				})
			);
		})
		->ifError(function($inner) use ($name) {
		...
		->ifSuccess(function($v) use ($name) {
			return new Success(new NamedValidator($name, new Success($v)));
		})
		->ifError(function($v) use ($name) {
			return new Success(new NamedValidator($name, new Failure($v)));
		})
		->ifSuccess(function($v) {
			return $v;
		});
	...
	function ifSuccess(callable $x) {
		return self::readdName(
			$this->removeName()->ifSuccess($x),
			$this->name
		);
	}
	function ifError(callable $x) {
		return self::readdName(
			$this->removeName()->ifError($x),
			$this->name
		);
	}
	function dename() {
