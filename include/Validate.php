...
abstract class Validate {
	...
	# Similar to ifOk(), but it binds a Failure within a Success;
	# it can be used to  make sure a value is a Result::none.
	function ifNone(callable $x) {
		return $this->ifSuccess(function($val) use($x) {
			return $val
				->ifSuccess(function($data) {
	...
	function filterFilterVar($cons, $msg) {
		return $this->ifOk(function($x) use ($cons, $msg) {
	...
	function filterNumber($integer) {
		return $this->ifOk(function($x) use ($integer) {
	...
	function requiredMaybe($enable) {
		return $this->ifNone(function($x) use ($enable) {
			if($enable) {
	...
	function mustBeTrue($enable) {
		return $this->ifOk(function($x) use ($enable) {
			if($enable && !$x) {
	...
	function mustHaveDomain($checkDomain) {
		return $this->ifOk(function($x) use ($checkDomain) {
			if($checkDomain !== null) {
	...
	function matchRegex($regex) {
		return $this->ifOk(function($x) use ($regex) {
	...
	function matchHash($hash) {
		return $this->ifOk(function($x) use ($hash) {
			if($hash !== null) {
	...
	function minMaxTime($min, $max) {
		...
		return $this->ifOk(function($x) use($min, $max, $compmin, $compmax) {
	...
	function stepNumber($step) {
		if($step === 'any') {
			...
			return $this->ifOk(function($x) use ($step) {
	...
	function stepDateTime($step) {
		if($step === 'any') {
			...
			return $this->ifOk(function($date) use ($step) {
	...
	function byName($name) {
		return $this->ifOk(function($v) use($name) {
	...
	function listValidate($minItems, $maxItems, $name, $items) {
		return $this->ifOk(function($list) use ($minItems, $maxItems, $name, $items) {
			...
			foreach($indices as $index) {
				...
				$result = $result
					->ifOk(function($soFar) use($validationResult, $index) {

						return $validationResult
							->ifOk(function($fieldResult) use($soFar, $index) {
...
# ifNone(), ifOk(), and collapse(). These methods are discussed above.
